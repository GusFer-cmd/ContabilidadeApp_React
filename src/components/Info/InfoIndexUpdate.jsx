...
function InfoIndexUpdate() {
    ...
    const fetchData = async () => {
        ...
        // Pega tudo de infos, cards e renters ao mesmo tempo.
        ...
        try {
          ...
          if (infosSnap.exists()) {
              // infos, cards, renters agora são objetos JS com os dados de cada coleção.
              const infos = infosSnap.val();
              const cards = cardsSnap.exists() ? cardsSnap.val() : {};
              const renters = rentersSnap.exists() ? rentersSnap.val() : {};

              // dados em pares [id, objeto]
              // filter mantém apenas os infos cujo card_id for igual ao cardId recebido da URL
